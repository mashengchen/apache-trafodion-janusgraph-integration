...
Also there need provide a yaml formatted file to give host and port. An example of yaml content named conf.yaml as follow:<br/>
```
hosts: [192.168.0.11]
port: 8182
```
...
* function ddl
```sql
create table_mapping function graph_query(query varchar(1000) , yamlFile varchar(100))
...
'g.V().has("personid",%1$s).outE("role").has("roletype",eq("director")).inV().as("source").values("movieid").as("movies").select("source").inE().has("roletype",eq("actor")).outV().has("personid",%1$s).select("movies")',
'/home/xxx/conf.yaml' ))) j
...
* function ddl
```sql
create table_mapping function graph_update(query varchar(1000) , yamlFile varchar(100))
...
Only two parameters are expected by the TMUDFs: the gremlin query and path of conf.yaml file.<br/>
<br/>
